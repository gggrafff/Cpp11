...
## Кратко: что такое семантика перемещения?
...
К счастью, во многих ситуациях современные компиляторы проводят оптимизации и избегают лишних копирований без нашего участия. Подробнее об этом:  
...
В этом примере заботу о лишних копиях берёт на себя компилятор, пон проведёт необходимые оптимизации, чтобы избежать лишних копирований. **Нельзя строить логику работы на количествах вызовов конструкторов/деструкторов**, потому что создания объектов могут быть соптимизированы компилятором, даже если конструктор/деструктор имеет наблюдаемые побочные эффекты (делает что-то кроме создания/удаления объекта).  

Но компилятор поможет нам не во всех ситуациях. Например:  
```C++
std::vector<BigObject> objects;
for (int i=0; i<3; ++i) {
    BigObject value;
    value.doSomethingForAdditionalInitialization();
    objects.push_back(value);  // Копирование, а хотелось бы переместить, не копируя
}
```
Хотелось бы иметь возможность подсказать компилятору, что объект больше не нужен и его можно переместить в новый контекст, а не копировать.  
